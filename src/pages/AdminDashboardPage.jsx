...
import AdminEditBooking from "./AdminEditBooking.jsx";
...
const DashboardPage = () => {
  ...
  const getAllBookings = async () => {
    await handledAPIGet("/booking/get/all")
      .then((response) => {
        if (response.data.bookings.length > 0) {
          setUserBookingData(response.data.bookings);
        }
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <>
      ...
      {isEditBooking ? <AdminEditBooking /> : null}
